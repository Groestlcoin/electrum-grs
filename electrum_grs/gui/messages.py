...
MSG_TERMS_OF_USE = (
"""1. Electrum-GRS is distributed under the MIT licence by Groestlcoin Developers. Most notably, this means that the Electrum-GRS software is provided as is, and that it comes without warranty.

2. We are neither a bank nor a financial service provider. In addition, we do not store user account data, and we are not an intermediary in the interaction between our software and the Groestlcoin blockchain. Therefore, we do not have the possibility to freeze funds or to undo a fraudulent transaction.
