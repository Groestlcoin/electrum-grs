...
from electrum_grs import bitcoin, keystore
...
from electrum_grs.lnchannel import Channel, ChannelBackup, ChannelState
from electrum_grs.lnonion import OnionPacket, OnionRoutingFailure, OnionFailureCode
from electrum_grs.mpp_split import SplitConfig, SplitConfigRating
from electrum_grs.crypto import sha256, pw_encode_with_version_and_mac
from electrum_grs.simple_config import SimpleConfig
from electrum_grs.transaction import Transaction, TxOutpoint, BCDataStream
from electrum_grs.util import bfh, UserFacingException
...
class TestLNWallet(ElectrumTestCase):
    ...
    async def test_trampoline_invoice_features_and_routing_hints(self):
        ...
        self.addCleanup(lambda: electrum_grs.trampoline._TRAMPOLINE_NODES_UNITTESTS.clear())
        ...
        electrum_grs.trampoline._TRAMPOLINE_NODES_UNITTESTS['regular_peer'] = LNPeerAddr(
            host="127.0.0.1",
            port=9735,
        ...
        del electrum_grs.trampoline._TRAMPOLINE_NODES_UNITTESTS['regular_peer']
        wallet.clear_invoices_cache()
    ...
    async def test_open_channel_just_in_time_failure_preimage_timeout(self):
        ...
        with mock.patch('electrum_grs.lnworker.LN_P2P_NETWORK_TIMEOUT', 0.01):
            with self.assertRaises(OnionRoutingFailure):
                await wallet.open_channel_just_in_time(
                    next_peer=next_peer,
                    next_amount_msat_htlc=1000000,
    ...
    async def test_open_channel_just_in_time_failure_broadcast(self):
        ...
        with mock.patch('electrum_grs.lnworker.ZEROCONF_TIMEOUT', 0.01), \
             mock.patch('electrum_grs.lnworker.asyncio.sleep', new_callable=mock.AsyncMock):
             with self.assertRaises(OnionRoutingFailure):
                await wallet.open_channel_just_in_time(
                    next_peer=next_peer,
                    next_amount_msat_htlc=1000000,
