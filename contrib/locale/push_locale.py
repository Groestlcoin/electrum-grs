...
locale_dir = os.path.join(project_root, "electrum_grs", "locale")
...
# add .py files
files_list = glob.glob("electrum_grs/**/*.py", recursive=True)
...
files_list = glob.glob("electrum_grs/gui/qml/**/*.qml", recursive=True)
files_list = sorted(files_list)  # makes output deterministic across CI runs
...
#  sed from " ../../gui/qml/"
#      to   " electrum/gui/qml/"
cmd = ["sed", "-i", r"s/ ..\/..\/gui\/qml\// electrum_grs\/gui\/qml\//g", f"{build_dir}/messages_qml.pot"]
subprocess.check_output(cmd)
...
os.chdir(os.path.join(project_root, "electrum-grs"))
